...
require_non_default_secret "AUTH_PASSWORD_HASH"
require_non_default_secret "AUTH_SESSION_SECRET"
require_non_default_secret "BROKER_CREDENTIALS_KEY"

auth_require_login="$(read_env "AUTH_REQUIRE_LOGIN")"
[[ "${auth_require_login,,}" == "true" ]] || die "AUTH_REQUIRE_LOGIN must be true on a Pi deploy"

auth_hash="$(read_env "AUTH_PASSWORD_HASH")"
[[ "$auth_hash" == scrypt\$v1\$* ]] || die "AUTH_PASSWORD_HASH must be generated by scripts/create_auth_hash.py"

broker_key="$(read_env "BROKER_CREDENTIALS_KEY")"
[[ "$broker_key" == gAAAAA* ]] || die "BROKER_CREDENTIALS_KEY must be generated by scripts/create_broker_key.py"

environment="$(read_env "ENVIRONMENT")"
[[ "$environment" == "development" ]] && die "ENVIRONMENT=development is not allowed for a Pi production deploy"

allowed_ips="$(read_env "ALLOWED_IPS")"
[[ "$allowed_ips" == *"10.8.0.0/24"* ]] || die "ALLOWED_IPS must include the WireGuard VPN subnet"
...
echo "Access:  https://10.8.0.1 (WireGuard VPN only)"
